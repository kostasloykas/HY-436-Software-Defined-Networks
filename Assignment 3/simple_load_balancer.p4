...
struct metadata {
    ...
    ip4Addr_t   srcIP; 
    ip4Addr_t   dstIP;
    ip4Addr_t   first_ip_that_pinged; 
...
parser SLBParser(packet_in packet,
    ...
    state parse_ethernet {
        packet.extract(hdr.ethernet); /* extract ethernet */

        /* select the next state according to etherType 
        if no match then reject parsing */
...
control SLBIngress(inout headers hdr,
    ...
    action set_client_metadata(ip4Addr_t firstAllowedReplica, ip4Addr_t lastAllowedReplica) {
        meta.isClient = 1;
        meta.isServer = 0;
        meta.first_ip_that_pinged = hdr.ipv4.dstAddr;
        random<ip4Addr_t>(meta.dstIP, firstAllowedReplica, lastAllowedReplica);
        /* change dstIP with the new one in order to apply in dst group membership table */
        hdr.ipv4.dstAddr = meta.dstIP;
        // hash( result , HashAlgorithm.random , firstAllowedReplica , { firstAllowedReplica , lastAllowedReplica } , lastAllowedReplica); /* select random server and store to metadata*/
    }
    ...
    table ipv4_servers {
        ...
        actions = {
            set_server_metadata;
            unset_server_metadata; /* default action */
        }
    }
    ...
    apply {
        ...
        if (!(hdr.arp.isValid() || hdr.ipv4.isValid())) {
            ...
            /* transform arp request to reply and store info in metadata for the arp_reply packet*/
            arp_request_to_reply( hdr.arp.hwSrcAddr , lbMAC , hdr.arp.protoSrcAddr, hdr.arp.protoDstAddr);

        }else if (hdr.ipv4.isValid()) {

            /* apply ipv4_clients and ipv4_servers tables in order to find if packet is from server or client*/
            ipv4_clients.apply();
            ipv4_servers.apply();
            ...
            if (!((meta.isClient == 1) || meta.isServer == 1) || meta.srcGroup != meta.dstGroup) {
                ...
                return;                                                     // or if the src/dst groups differ
            }                                                               
            else {
                
                if(meta.isClient == 1){
                    /* if client doesn't ping service ip drop packet */
                    if(meta.first_ip_that_pinged != serviceIP){
                        drop();
                        return;
                    }
                    /* store the lbMac in metadata to service ip mac address */
                    meta.srcMAC = lbMAC;
                    
                }else if(meta.isServer == 1){
                    /* just change in egress the srcMAC and srcIP to service IP and MAC 
                    for transparensy */
                }
                /* sets the egress destination port and destination MAC */
...
control SLBEgress(inout headers hdr,
    ...
    action rewrite_client_to_server() {
        hdr.ipv4.dstAddr = meta.dstIP;
        hdr.ethernet.srcAddr = meta.srcMAC;
    ...
    action update_packet_headers_for_arp_reply(){
        
        hdr.arp.hwSrcAddr = meta.srcMAC;
        hdr.arp.protoSrcAddr = meta.srcIP;
        hdr.arp.hwDstAddr = meta.dstMAC;
        hdr.arp.protoDstAddr = meta.dstIP;
        hdr.ethernet.dstAddr = meta.dstMAC;
        hdr.ethernet.srcAddr = meta.srcMAC;
        hdr.arp.opCode = 2; /* reply opcode */
    }
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            update_packet_headers_for_arp_reply();
        }
    }
}
